...
## G. 지연 축 심층 추적 (2026-07-16 — "끝까지" 3차, fastest)

1. **[미해결·큰것] 네이티브 오디오 지연 ~200~450ms** — `audio.rs` WASAPI
   **shared 200ms 버퍼**(`client.Initialize(..., 2_000_000, ...)` = 2M×100ns)
   + **250ms drop-oldest FIFO**(`fifo_cap = rate/4`). 정상 상태 지연이 수백 ms.
   **필드 리포트 "오디오 지연 체감"(2026-07-15)의 정량 근원.** 근본 해결 =
   Phase B WASAPI **exclusive event-driven**(~10ms, 128프레임). 중간 완화 =
   FIFO 캡·버퍼 축소(단 지터 흡수와 트레이드오프 — 라이브 튜닝).
2. **[미해결] present 프레임당 GPU↔CPU 왕복 ~5-8ms** — decode→`hwframe_transfer`
   (GPU→CPU)→swscale→D3D11 업로드(CPU→GPU). `Present(0)`=vsync-off는 정상.
   제로카피 NV12 텍스처 직결(Phase B, present.rs §2)이 지연·효율 동시 해결.
   ALLOW_TEARING/exclusive fullscreen은 마지막 vblank 절감(부차).
3. **[유의] swscale FAST_BILINEAR 크로마 업샘플** — 4:2:0→RGBA 크로마 2x
   업샘플이 fast(저품질). 색 엣지 fringing 미세 잔존. 제로카피 GPU 셰이더가
   근본책(고품질 크로마 무료). 스톱갭으로 SWS 플래그 상향 가능하나 CPU 지연
   가중 — GPU 경로 우선.

---

## H. 종합 — "기술적으로 가장 화질 좋고 빠르고 효율적"의 결정판 갭

세 축 각각, "타 대비 우위"로 **정의될 수밖에 없으려면** 닫혀야 하는 것:

### 화질(best) — 필요조건
- [x] 색 정확도(네이티브 709/range) — §F 수리
- [ ] 인코더 무료 점심(spatial AQ + preset P4/5 + weighted-pred) — 측정→활성
- [ ] 4:4:4(색 텍스트) — 클라 준비, 호스트 e2e
- [ ] 10-bit(밴딩) — 클라 준비, 호스트 e2e + **네이티브 10-bit present(R10G10B10A2)**
- [ ] QU 정지영역 무손실(U4 P2) — 데스크톱 픽셀-퍼펙트
- [ ] (선택) 클라 슈퍼레졸루션 — 저해상도→고화질

### 지연(fastest) — 필요조건
- [x] 프레임딜레이 CC + FEC(손실 중 무붕괴) — ultra 코어
- [x] 스톨 복구 사다리 — M4
- [ ] **네이티브 오디오 exclusive(~10ms)** — §G #1, 현재 수백 ms
- [ ] **제로카피 present(왕복 제거)** — §G #2
- [ ] 서브프레임 슬라이스 겹침(U3) — 인코드→프레젠트 지연
- [ ] Gate C 외부 input-to-photon 실측 — 왕좌 판정

### 효율(most efficient) — 필요조건
- [x] ABR + CC + 적응 FEC 비율(P3 컨트롤러) — 대역 추종
- [ ] HEVC/AV1 기본(−30~50% 비트) — 클라 준비, 호스트 e2e
- [ ] 무료 점심(spatial AQ = 지각 배분)
- [ ] QU 정지영역 무손실(정적 대역 ~0)
- [ ] 동적 해상도 스케일링(혼잡 시 해상도↓+업스케일)
- [ ] (선택) AV1 필름그레인 합성

**결론:** "기술적 최강"의 골격(지연 코어·FEC·색 정확도)은 섰다. 남은
결정판 갭은 대부분 **① 호스트 인코드 활성(무료 점심+코덱+4:4:4/10-bit,
