            ...
            if let Some(c) = self.inflight_queue.pop(i) {
                if !c.acked {
                    ...
                    if c.nsent == 1 && sna32gte(c.tsn, self.min_tsn2measure_rtt) {
                        ...
                        // BETTERPARSEC PATCH (2026-07-17): SystemTime is not
                        // monotonic — an NTP step backwards mid-session made
                        // duration_since fail here, and returning
                        // ErrInvalidSystemTime aborted the whole SACK handler
                        // with the inflight queue already partially popped,
                        // desyncing the association (endless "unable to be
                        // popped from inflight queue TSN" — every DataChannel
                        // frozen). A clock jump only invalidates this one RTT
                        // sample, so skip the measurement instead of failing.
                        if let Ok(rtt) = SystemTime::now().duration_since(c.since) {
                            ...
                            log::warn!(
                                "[{}] SACK: wall clock stepped backwards — skipping RTT sample",
                                self.name
                            );
                        }
                    }
            ...
            for i in g.start..=g.end {
                ...
                if let Some(c) = self.inflight_queue.get(tsn) {
                    if !is_acked {
                        ...
                        if c.nsent == 1 {
                            self.min_tsn2measure_rtt = self.my_next_tsn;
                            // BETTERPARSEC PATCH (2026-07-17): same as the
                            // cumulative-ack site above — a wall-clock step
                            // must skip this RTT sample, never abort the SACK.
                            if let Ok(rtt) = SystemTime::now().duration_since(c.since) {
                                let srtt = self.rto_mgr.set_new_rtt(rtt.as_millis() as u64);
                                log::trace!(
                                    "[{}] SACK: measured-rtt={} srtt={} new-rto={}",
