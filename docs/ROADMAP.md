...
- [~] **네이티브 클라 실행 버스트 + 인천 배포판 패키징** (2026-07-16 오전, 헤드리스 빌드·검증 — 7커밋 9f2912b…9def901, 워킹트리 클린): 인천 물리 모니터 필드 배포를 겨냥한 app-native 실행 묶음.
  ...
  **중대 발견 — 네이티브 와이어 이중 송신 ≈ 명목×2.2** (2026-07-16 오후, 07-16e 30초-끊김 필드 리포트 추적 — "Parsec은 50M도 안정"이라는 owner 반박이 촉발): `streamer/src/transport/webrtc/video.rs::send_decode_unit`이 프레임마다 ⑴ FEC 활성 시(네이티브는 비디오가 video_fec 데이터채널이라 **항상**) 풀프레임을 FEC로 enqueue(+리페어 ~20%) 하고 ⑵ **같은 프레임을 RTP 트랙으로도 무조건 packetize·송신**(네이티브는 그 트랙을 소비하지 않고 버림). 즉 네이티브 세션 실 와이어 ≈ **명목×2.2** — 명목 20M = 와이어 ~44M **고정**(적응 없음: ABR 피드백(REMB/RR)은 네이티브가 안 쓰는 RTP 경로 기준 + 적용 자체가 Foundation 동적 비트레이트 호스트 필요, stock이면 영구 고정). Parsec "50M 안정"은 BUD가 상한 아래서 실시간 적응하는 것이라 비교 자체가 부당 — 고정 44M vs 적응 ≤50M. 8M 기본(와이어 ~17.6M)이 역대 안정 세션과 정합. **후속 트랙**: ⑴ [중형] 송신 dedup — FEC-primary 클라(StartStream 플래그)에 RTP 트랙 송신 생략(와이어 절반, 프로토콜+streamer+client-transport, serde(default) 호환 주의) ⑵ [중형] 데이터경로 적응 — RxCore/FEC-ack 채널로 클라 수신 피드백(손실/지연)→ABR·FecRatioController(기계 존재, 시그널만 배선) ⑶ [라이브] Foundation ACK 호스트 상시화(0x5506 적용 전제).
